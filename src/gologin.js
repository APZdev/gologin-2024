...
export class GoLogin {
  ...
  convertPreferences(preferences) {
    ...
    if (get(preferences, "navigator.language")) {
      ...
      preferences.languages = get(preferences, "navigator.language").replace(/;|q=[\d.]+/gim, "");
    }
  ...
  async createBrowserExtension() {
    ...
    await decompress(extension_source, extPath, {
      ...
      .then(() => {
        ...
        return writeFile(join(extPath, "uid.json"), JSON.stringify({ uid: that.profile_id }, null, 2)).then(
          () => extPath
        );
      })
      .catch(async (e) => {
  ...
  async createStartup(local = false) {
    ...
    if (allExtensions.length) {
      ...
      if (ExtensionsManagerInst.useLocalExtStorage) {
        ...
        profileExtensionsCheckRes = (profileExtensionPathRes?.profileExtensionsCheckRes || []).concat(
      ...
      this.extensionPathsToInstall = ExtensionsManagerInst.getExtensionsToInstall(
        extSettings,
  ...
  async spawnBrowser() {
    ...
    if (this.vnc_port) {
      ...
      execFile(script_path, [ORBITA_BROWSER, remote_debugging_port, proxy, profile_path, this.vnc_port, tz], { env });
    } else {
      let params = [
  ...
  async create(options) {
    ...
    if (response.statusCode === 400) {
      throw new Error(
        `gologin failed account creation with status code, ${response.statusCode} DATA  ${JSON.stringify(
